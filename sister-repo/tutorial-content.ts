...
export const TUTORIAL_DEVICES: TutorialDevice[] = [
  {
    ...
    steps: [
      ...
      { text: "Open IPTV Smarters Pro, pick Login with Xtream Codes API and enter the username, password and server URL from your Best IPTV UK welcome email." },
  ...
  },
  {
    ...
    appImage: "/best-iptv-uk-xott.webp",
    steps: [
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "Choose \"Login with Xtream Codes API\" (do NOT choose \"Load Your Playlist or File/URL\" — that's M3U mode)." },
      {
        ...
        substeps: [
          "Any Name: Best IPTV UK (or whatever you prefer)",
    ...
    troubleshooting: [
      "Smarters Pro reports \"Server not found\": check the Server URL field contains only the base host http://line.trxdnscloud.ru — nothing longer, no get.php?... query string, no /player_api.php. The M3U URL is different from the Xtream Codes server URL.",
      ...
      "Home wi-fi drops during streaming: Apple TV 4K supports wi-fi 6 (2022 model) or wi-fi 5 (2017, 2021). Move to 5GHz band in the router config, or use the Ethernet port on the 2022 model for guaranteed 4K UHD stability.",
    ],
  },
  {
    slug: "android-tv",
    ...
    steps: [
      ...
      { text: "Type the username, password and server URL from your Best IPTV UK welcome email." },
    ],
    troubleshooting: [
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "Enter the username, password and server URL from your Best IPTV UK welcome email." },
    ],
    troubleshooting: [
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "Open it, choose Login with Xtream Codes API and enter your username, password and server URL from the Best IPTV UK welcome email." },
    ],
    notes: [
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "Sign in with the Xtream Codes username, password and server URL from your Best IPTV UK welcome email." },
  ...
  },
  {
    ...
    troubleshooting: [
      "Portal shows \"Connecting…\" but never completes: check the Portal URL field contains only the base host http://line.trxdnscloud.ru — nothing longer, no get.php?... query string. Formuler MyTVOnline requires the Xtream Codes API URL, not the M3U URL.",
  ...
  },
  {
    ...
    metaTitle: "IPTV on Roku — Hot Player Setup Guide (2026)",
    metaDescription:
      "IPTV setup on Roku in 2026 using Hot Player from the Channel Store. MAC-address activation via WhatsApp support. Works on Roku TV and Streaming Stick.",
    intro:
      "Roku is one of the trickier IPTV platforms because Roku's own operating system doesn't support the third-party apps that dominate Android TV and Fire TV. The workaround is Hot Player from the Roku Channel Store: install the app, note the MAC Address it displays, send it to our WhatsApp support team with your Xtream Codes credentials, and we register your device on the back-end. Once we confirm activation, refresh Hot Player and your channels load. This works on every Roku device: Roku TV (any brand — TCL, Hisense, Sharp), Roku Streaming Stick (all generations), Roku Ultra, Roku Express. The trade-off with Roku is that the interface is more basic than IPTV Smarters Pro on Android/Windows — Hot Player is functional but not polished. If you have a choice between a Roku and a Fire Stick 4K Max at the same price, the Fire Stick is a better IPTV device. But if you already own Roku hardware, Hot Player works reliably and the activation flow via WhatsApp takes about five minutes.",
    compatibility:
      "All current Roku devices support Hot Player from the Channel Store: Roku TVs from 2018 onwards (any manufacturer), Roku Streaming Stick 4K, Roku Streaming Stick 4K+, Roku Ultra, Roku Express 4K. Older Roku Express and Roku Premiere (pre-2018) run Hot Player but sometimes struggle with 4K UHD streams because the chipset is limited. Roku 3 and older aren't recommended — they can install the app but the interface is too slow to be usable.",
    ...
    troubleshooting: [
      "Hot Player missing from the Channel Store: search under \"Hot Player IPTV\" specifically. Roku's search sometimes misses the app under the plain name.",
      "App displays \"Registration Pending\" indefinitely: this means we haven't received your MAC address on WhatsApp yet — send it to +44 7878 757831 with your account email and we'll activate.",
      "Streams work on some channels but 4K UHD channels fail: Roku Streaming Stick 4K decodes HEVC in hardware but throttles when the chip gets warm. If it's in a wall-mounted TV's cavity, move it to an open shelf for airflow.",
      "Channel list won't refresh after operator activation: force-close Hot Player (from Roku home, highlight the app, press * on the remote → Remove channel → wait 10 seconds → re-add from Channel Store). This clears the local cache and pulls a fresh list.",
      "Playback stutters despite good wi-fi: Roku's wi-fi tuning is more aggressive than other platforms. Try a wired Ethernet connection via a Roku Ultra or the powered USB adapter on Streaming Stick 4K+.",
    ],
  },
  {
    slug: "buzztv",
    ...
    troubleshooting: [
      ...
      "Portal URL rejected: enter the URL exactly as http://line.trxdnscloud.ru — nothing longer, no get.php?... at the end. BuzzTV's XC API expects the base host only.",
